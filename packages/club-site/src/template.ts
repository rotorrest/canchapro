...
<body class="bg-white">
  ...
  <section class="relative bg-brand text-white">
    <div class="max-w-5xl mx-auto px-5 py-20 text-center">
      ...
      <a href="#" class="mt-8 inline-block bg-white text-brand font-semibold px-8 py-3 rounded-xl hover:bg-gray-100 transition-colors">
        Reservar cancha
      </a>
    </div>
  </section>
  ...
  <section class="max-w-5xl mx-auto px-5 pb-16">
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
      ${c.photos.map((p) => `<img src="${p}" alt="${c.clubName}" class="rounded-xl w-full h-48 object-cover">`).join("\n      ")}
    </div>
  </section>` : ""}
